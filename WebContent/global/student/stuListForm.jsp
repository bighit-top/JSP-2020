...
%>
   <tr height="30">
    ...
    <td  width="250" >
      ...
      <a href="studentContentForm.jsp?num=<%=article.getNum()%>&pageNum=<%=currentPage%>">
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="stuListForm.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        <a href="stuListForm.jsp?pageNum=<%= i %>">[<%= i %>]</a>
<%
        }
        if (endPage < pageCount) {  %>
        <a href="stuListForm.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<%
        }
    }
%>
